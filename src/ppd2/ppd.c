...
int main()
{
	...
	char* mensaje = NULL;
	int32_t  thidConsola,thidAtenderpedidos, tam;
...
void escribirSect(int sect, char buffer[512], nipc_socket sock)
{
	...
	nipc_packet resp;
		...
		if((0 <= sect) && (cantSect >= sect))
		{
			...
			msync(dirSect, TAM_SECT, MS_ASYNC);
			//if(0 != munmap(dirMap,TAM_PAG))
				//printf("Fallo la eliminacion del mapeo\n");
...
void * discoMap(int32_t sectores, int dirArch)
{
	div_t res;
	int32_t ret, tam;
	res = div(sectores, 8);
	...
	if(ret = posix_madvise(diskmap, tam, POSIX_MADV_SEQUENTIAL))
...
void traceSect(int32_t sect, int32_t nextsect, int cliente)
{
	char * bufferConsola;
	double tiempo;
